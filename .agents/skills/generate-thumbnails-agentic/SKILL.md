...
1. **Playwright MCP is configured.** Run a Playwright MCP tool (e.g. `browser_navigate`) to verify. If unavailable, **STOP** and ask the user:
   ...
   > ```Expand commentComment on lines R20 to R22Resolved
   >
...
```Expand commentComment on lines R354 to R356Resolved
...
```Expand commentComment on lines R425 to R427Resolved
...
```Expand commentComment on lines R436 to R437Resolved
